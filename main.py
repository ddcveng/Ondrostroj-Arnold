...
for i in range(8):
    for j in range(8):
        ...
        if piece:
            img = pyglet.resource.image(f"{type(piece).__name__}_{piece.color}.png")
...
def on_draw():
    ...
    #background
    pyglet.graphics.draw(4, pyglet.gl.GL_QUADS, ('v2f', [0,0, WIDTH,0, WIDTH,HEIGHT, 0,HEIGHT]), ('c3B', [128, 0, 128]*4))
    #chessboard
    pyglet.graphics.draw(4, pyglet.gl.GL_QUADS, ('v2f', [OFFSET,OFFSET, WIDTH-OFFSET,OFFSET, WIDTH-OFFSET,HEIGHT-OFFSET, OFFSET,HEIGHT-OFFSET]), ('c3B', [191,112,48]*4))
    pyglet.graphics.draw(128, pyglet.gl.GL_QUADS, ('v2f', coords), ('c3B', [255,255,255]*128))
    #avilable moves
    pyglet.graphics.draw(game.num_vertexes, pyglet.gl.GL_QUADS, ('v2f', game.highlight_coords), ('c3f', [0, 255, 0]*game.num_vertexes))
    ...
    if game.board.check:
        ...
        pyglet.graphics.draw(4, pyglet.gl.GL_QUADS, ('v2f', [kingx*GRID_SIZE+OFFSET, kingy*GRID_SIZE+OFFSET,
                                                            kingx*GRID_SIZE+OFFSET, (kingy+1)*GRID_SIZE+OFFSET,
                                                            (kingx+1)*GRID_SIZE+OFFSET, (kingy+1)*GRID_SIZE+OFFSET,
    ...
    if game.board.is_on_board(pos):
        if not game.active_piece:
            ...
            if pos in game.highlights: #if the clicked square is highlited, then move there
                game.active_piece.sprite.x = x*GRID_SIZE+OFFSET+(GRID_SIZE-img.width)/2
                game.active_piece.sprite.y = y*GRID_SIZE+OFFSET+(GRID_SIZE-img.height)/2
                ...
                if game.board.can_endanger(enemy_king.pos, game.active_piece):
                    ...
                    game.over = game.checkmate()
                elif game.board.check:
                    game.board.check = None
                
                if type(game.active_piece) == Pawn and not game.active_piece.moved_once:
                    game.active_piece.moved_once = 1 
